...
from video_record import VideoRecorder  # for video logging
...
def train():
    ...
    # Create a list of agents for video logging
    agents = [ppo_agent, sac_agent]
    ...
    # Frequency for video logging (adjust as needed)
    EPOCH_CHECK = 10
    ...
    for iteration in progress_bar:
        ...
        for epoch in range(wandb.config.ppo_num_epochs):
            ...
            new_logprobs_sum = new_logprobs.sum(dim=-1)  # shape becomes [ppo_num_steps]
            ppo_logprobs_sum = ppo_logprobs.sum(dim=-1)    # shape becomes [ppo_num_steps]
        ...
        if iteration % EPOCH_CHECK == 0 and iteration > 0:
            ...
            # Pass the agents list to the video recorder
            # pdb.set_trace()
            video_recorder.start_recording(agents, iteration, mode='agent', algorithm='sac_vs_ppo_agent')
            display_manager.set_headless()
    ...
    # Cleanup video recordings at the end of training
    video_recorder.cleanup()
